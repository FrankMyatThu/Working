...
namespace WebApplication1.Controllers
{
    public class EmployeeController : Controller
    {
        ...
        [HttpPost]
        public ActionResult Create(tbl_EmpDep _tbl_EmpDep, HttpPostedFileBase _txtFile)
        {
            using (InterviewDBEntities _InterviewDBEntities = new InterviewDBEntities())
            {
                if (_txtFile != null)
                {
                    _txtFile.SaveAs(Server.MapPath("~/Image/") + _txtFile.FileName);
                    _tbl_EmpDep.EmployeePhoto = _txtFile.FileName;
                }
        ...
        public ActionResult Index()
        {
            ...
            { 
                List_tbl_EmpDep = (from _tbl_EmpDep in _InterviewDBEntities.tbl_EmpDep
                                        group _tbl_EmpDep by _tbl_EmpDep.DepartmentID into goup_tbl_EmpDep
                                        select goup_tbl_EmpDep.OrderByDescending(x => x.JoinDate).FirstOrDefault()).ToList<tbl_EmpDep>();
            }
        ...
        [HttpPost]
        public ActionResult Update(tbl_EmpDep _tbl_EmpDep, HttpPostedFileBase _txtFile)
        {
            using (InterviewDBEntities _InterviewDBEntities = new InterviewDBEntities())
            {
                ...
                {
                    _txtFile.SaveAs(Server.MapPath("~/Image/") + _txtFile.FileName);
                    db_tbl_EmpDep.EmployeePhoto = _txtFile.FileName;
                }
                _InterviewDBEntities.SaveChanges();
            }
